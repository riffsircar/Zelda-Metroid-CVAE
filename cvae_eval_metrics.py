...
def v_symmetry(level):
    total = 0
    level_no_mid = level[:len(level)//2] + level[(len(level)//2)+1:]
    level_1, level_2 = level_no_mid[:len(level_no_mid)//2],level_no_mid[len(level_no_mid)//2:]
    for l1,l2 in zip(level_1,reversed(level_2)):
        for a,b in zip(l1,l2):
            if a == b:
                total += 1
    return total
